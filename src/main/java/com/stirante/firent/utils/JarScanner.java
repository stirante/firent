...
public class JarScanner {
    ...
    public static void registerListeners(JavaPlugin plugin, File file, String scannedPackage) {
        //get all classes inside specified package and iterate through them
        List<Class<?>> classes = scan(plugin, file, scannedPackage);
        for (Class<?> aClass : classes) {
            ...
            if (Listener.class.isAssignableFrom(aClass)) {
                try {
                    ...
                    Listener l =
                            (Listener) aClass.newInstance();//TODO: iterate through constructors and find also JavaPlugin constructor
                    //register listener
                        ...
                        if (method.getParameterCount() == 1 &&
                            ...
                            plugin.getLogger()
                                    .warning("Method " + method.getName() +
                                            " looks like event handler, but is missing EventHandler annotation");
                        }
                    }
     ...
     */
    public static List<Class<?>> scan(JavaPlugin plugin, File file, String scannedPackage) {
            ...
            while (it.hasMoreElements()) {
                ...
                if (next.getName().startsWith(s) && next.getName().endsWith(".class")) {
                    try {
                        ...
                        plugin.getLogger().log(Level.WARNING, "Couldn't find class " + next.getName(), e);
                    }
                }
            }
        } catch (IOException e) {
            plugin.getLogger().log(Level.SEVERE, "Couldn't access plugin jar", e);
        }
        return classes;
    }
